...
class ColorUtils:
    ...
    @staticmethod
    def evaluate_iris_and_pupil_rad(iris: Iris) -> (int, int):
    ...
    def process_image(image: np.ndarray):
        ...
        """
        points = ColorUtils.get_points(image)
        left = Iris(points, align='left')
        right = Iris(points, align='right')

        mask = np.array(ColorUtils.make_mask(image, left) + ColorUtils.make_mask(image, right), dtype=bool)
        b, g, r = cv2.split(image)

        return np.median(r[mask]), np.median(g[mask]), np.median(b[mask]), left, right

    @staticmethod
    def plot_image_and_detected_color(image: np.ndarray, savepath: str):
        r, g, b, left, right = ColorUtils.process_image(image)
        ...
        iris_radius, pupil_radius = ColorUtils.evaluate_iris_and_pupil_rad(left)
        ...
        iris_radius, pupil_radius = ColorUtils.evaluate_iris_and_pupil_rad(right)
